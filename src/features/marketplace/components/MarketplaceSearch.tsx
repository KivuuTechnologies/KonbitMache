...
export function MarketplaceSearch({ locale, onSearch, activeFilters, embedded = false, showHero = true }: MarketplaceSearchProps) {
  ...
  const content = (
    <div className="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <div
        className={`${showHero ? 'mt-7' : 'mt-6'} flex flex-col gap-1 rounded-2xl border bg-surface p-1.5 shadow-sm md:grid md:grid-cols-[minmax(0,1fr)_auto]`}
        role="search"
      >
        <label className="flex min-h-14 min-w-0 items-center gap-2 rounded-xl px-3 focus-within:bg-surface-muted">
          <svg className="h-5 w-5 shrink-0 text-muted" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="mt-5 lg:hidden">
        ...
        >
          <svg className="h-5 w-5 shrink-0" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            <path d="M2 22h20M6 18V11M10 18V11M14 18V11M18 18V11M12 2 2 8v2h20V8z" />
          </svg>
          {t.nav.publish}
        </a>
      </div>
  ...
  return embedded ? content : <section id="buscar" className="border-b bg-surface py-7 sm:py-12">{content}</section>;
}
